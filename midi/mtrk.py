...
class MTrk(object):
  ...
  #--------------------------------------------------------------------
  def __ParseFileEvent(self, data):
    ...
    event = None
    if sByte == 0xff:
      event = self.__ParseFileMetaEvent(data, dTime)
    elif (sByte & 0xf0) == 0xf0:
      event = self.__ParseFileSystemEvent(data, dTime)
    else:
  ...
  def __ParseFileMetaEvent(self, data, dtime):
    ...
    params = MTrk.MetaEventDict[eType]
  ...
  def __ParseFileSystemEvent(self, data, dtime):
    ...
    params = MTrk.SystemEventDict[eType]
